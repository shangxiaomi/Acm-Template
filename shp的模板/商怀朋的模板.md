...
- [1. 图论（待整理）](#1-图论待整理)
    - [1.1. 最短路](#11-最短路)
        - [1.1.1. 迪杰斯特拉（堆优化）](#111-迪杰斯特拉堆优化)
        - [1.1.2. 生成最短路地图](#112-生成最短路地图)
        - [1.1.3. 分层图最短路](#113-分层图最短路)
    - [1.2. 最小生成树](#12-最小生成树)
    - [1.3. 网络流](#13-网络流)
        - [1.3.1. 最大流最小割](#131-最大流最小割)
    - [1.4. 最小费用流(迪杰)](#14-最小费用流迪杰)
- [2. 计算几何（待学习）](#2-计算几何待学习)
- [3. Bitset](#3-bitset)
- [4. 极大团，最大团](#4-极大团最大团)
    - [4.1. 团的定义](#41-团的定义)
    - [4.2. 求最大团（原图的补图是二分图）](#42-求最大团原图的补图是二分图)
    - [4.3. 二分图的一些知识](#43-二分图的一些知识)
        - [4.3.1. 一些概念](#431-一些概念)
                - [4.3.1.0.1. 匹配](#43101-匹配)
                - [4.3.1.0.2. 边覆盖](#43102-边覆盖)
                - [4.3.1.0.3. 独立集](#43103-独立集)
                - [4.3.1.0.4. 顶点覆盖](#43104-顶点覆盖)
        - [4.3.2. 二分图的Hall's marriage theorem定理](#432-二分图的halls-marriage-theorem定理)
- [5. 蒙特卡洛随机](#5-蒙特卡洛随机)
    - [5.1. 牛客暑期多校训练营（第二场）A（随机化）](#51-牛客暑期多校训练营第二场a随机化)
- [6. 随机数种子设置](#6-随机数种子设置)
- [7. 字符串](#7-字符串)
    - [7.1. 字符串hash](#71-字符串hash)
- [8. 莫队算法](#8-莫队算法)
    - [8.1. 普通莫队算法](#81-普通莫队算法)
    - [8.2. 树上莫队算法(对欧拉序数组进行判断)](#82-树上莫队算法对欧拉序数组进行判断)
- [9. 单调栈和单调队列](#9-单调栈和单调队列)
    - [9.1. 单调队列（队尾插入，队首删除）](#91-单调队列队尾插入队首删除)
    - [9.2. 单调栈（队尾插入队尾删除）](#92-单调栈队尾插入队尾删除)
- [10. 笛卡尔树](#10-笛卡尔树)
    - [10.1. 定义](#101-定义)
    - [10.2. 性质](#102-性质)
    - [10.3. 建立](#103-建立)
- [11. 字符串相关](#11-字符串相关)
    - [11.1. 序列自动机](#111-序列自动机)
    - [11.2. 后缀自动机和回文自动机](#112-后缀自动机和回文自动机)
- [12. 三分查找算法](#12-三分查找算法)
- [13. 数学相关模板](#13-数学相关模板)
    - [13.1. 线性基](#131-线性基)
        - [13.1.1. 简单理解](#1311-简单理解)
        - [13.1.2. 前缀线性基](#1312-前缀线性基)
    - [13.2. 线性基求交](#132-线性基求交)
    - [13.3. 素数筛法](#133-素数筛法)
        - [13.3.1. 埃式筛法](#1331-埃式筛法)
        - [13.3.2. 欧式筛法](#1332-欧式筛法)
    - [13.4. 求逆元](#134-求逆元)
        - [13.4.1. 扩展欧几里得求逆元（mod可以不为质数，逆元不一定存在）](#1341-扩展欧几里得求逆元mod可以不为质数逆元不一定存在)
        - [13.4.2. 费马小定理求逆元(mod必须为质数！！！)](#1342-费马小定理求逆元mod必须为质数)
        - [13.4.3. 连续逆元(mod必须为质数！！！)](#1343-连续逆元mod必须为质数)
    - [13.5. 卢卡斯定理模板](#135-卢卡斯定理模板)
    - [13.6. 二分乘法](#136-二分乘法)
    - [13.7. 快速幂](#137-快速幂)
    - [13.8. 高斯消元](#138-高斯消元)
    - [13.9. 矩阵快速幂](#139-矩阵快速幂)
    - [13.10. 组合数（待学）](#1310-组合数待学)
    - [13.11. 蔡勒公式](#1311-蔡勒公式)
    - [13.12. 辗转相除法解不等式(待学习)](#1312-辗转相除法解不等式待学习)
- [14. RMQ](#14-rmq)
- [15. 树状数组](#15-树状数组)
        - [15.0.1. 一维](#1501-一维)
        - [15.0.2. 二维](#1502-二维)
- [16. 线段树](#16-线段树)
    - [16.1. 区间修改（加减）,区间求和](#161-区间修改加减区间求和)
    - [16.2. 无区间修改，查询区间内最小值和最大值](#162-无区间修改查询区间内最小值和最大值)
    - [16.3. 区间覆盖，查询有多少不同颜色（贴海报问题）](#163-区间覆盖查询有多少不同颜色贴海报问题)
    - [16.4. 区间每个数开根号（单点更新+剪枝），区间查询求和](#164-区间每个数开根号单点更新剪枝区间查询求和)
    - [16.5. 区间修改加法，乘法，置换，区间查询每个数的幂的和](#165-区间修改加法乘法置换区间查询每个数的幂的和)
    - [16.6. 区间修改是否覆盖，区间查询相邻剩余空间左右端点值](#166-区间修改是否覆盖区间查询相邻剩余空间左右端点值)
    - [16.7. 区间修改覆盖/清空，区间查询相应长的连续空间是否存在](#167-区间修改覆盖清空区间查询相应长的连续空间是否存在)
    - [16.8. 求二维图形的边长（数据离散化）](#168-求二维图形的边长数据离散化)
    - [16.9. 求二维空间的面积](#169-求二维空间的面积)
    - [16.10. 最大连续区间和](#1610-最大连续区间和)
    - [16.11. 斐波那契(待补题)](#1611-斐波那契待补题)
    - [16.12. 权值线段树](#1612-权值线段树)
    - [16.13. 区间转点](#1613-区间转点)
- [17. 主席树](#17-主席树)
    - [17.1. 区间不同的数的个数](#171-区间不同的数的个数)
    - [17.2. 静态第k个数字](#172-静态第k个数字)
    - [17.3. 树上路径第k大](#173-树上路径第k大)
    - [17.4. 动态第k大](#174-动态第k大)
    - [17.5. 区间修改，区间求和（自己写的代码未过）](#175-区间修改区间求和自己写的代码未过)
- [18. 最近公共祖先（最小公共祖先）（LCA）](#18-最近公共祖先最小公共祖先lca)
    - [18.1. 倍增法](#181-倍增法)
- [19. 最长上升子序列](#19-最长上升子序列)
- [20. 可撤销并查集](#20-可撤销并查集)
- [21. 差分](#21-差分)
- [22. 数位dp](#22-数位dp)
    - [22.1. 模板](#221-模板)
- [23. python3输入输出](#23-python3输入输出)
    - [23.1. 输入](#231-输入)
